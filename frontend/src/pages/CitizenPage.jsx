...
const COUNTRY_OPTIONS = ["India", "EU", "US", "UK"];
...
export default function CitizenPage() {
  ...
  const referencePct = data
    ? Math.round((data.reference_approval_rate || 0) * 100)
    : 0;
  const profileLabel =
    formData.domain === "lending"
    ...
    <section className="page-wrap">
      <header className="page-hero">
        ...
        <p className="hero-subtitle">
          Submit a demographic profile and receive a transparent, model-backed
          disparity report across hiring and lending outcomes.
        </p>
      </header>
        ...
        <article className="panel">
          ...
          {!result && !loading && (
            <div className="empty-state">
              ...
              <p>
                Run the check to generate disparity and compliance indicators
            ...
            <div>
              <div className="ratio-view">
                <p className="ratio-label">
                  Disparity Ratio vs Reference Group
                </p>
              ...
              >
                <span
                  className={`badge ${data.fourfifths_breach ? "error" : "success"}`}
                >
                  {data.fourfifths_breach ? (
                ...
                {counterfactuals && counterfactuals.length > 0 ? (
                  <div className="counterfactual-list">
                    {counterfactuals.map((item, index) => (
                      <div
                        key={`${item.changed_attribute}-${index}`}
                        className="counterfactual-card"
                      >
                        <p className="muted-text">
                          <strong>
                            {item.changed_attribute.replace(/_/g, " ")}
                          </strong>
                        </p>
                  ...
                  <p className="muted-text">
                    No significant counterfactual changes found.
                  </p>
                )}
              </section>
              {}
      ...
      <p className="footer-note">
        EquityGuard identifies statistical signals, not legal determinations.
        Use these outputs as a triage layer before formal policy or legal
        review.
      </p>
    </section>
  );
