...
function parseOutcome(value) {
  ...
  if (
    ["1", "true", "yes", "approved", "approve", "accept"].includes(normalized)
  ) {
    return 1;
  }
  if (
    ["0", "false", "no", "denied", "deny", "reject", "rejected"].includes(
      normalized,
    )
  ) {
    return 0;
...
export default function AuditPage() {
  ...
  const handleUpload = () => {
    ...
    Papa.parse(file, {
        ...
        try {
          const decisions = data.map((row) => {
            ...
            return {
              ...
              outcome: parseOutcome(
                readField(row, ["outcome", "Outcome", "decision", "Decision"]),
              ),
            };
          });
          ...
          setResult({
            error: true,
            message: "Audit failed. Please verify the CSV and try again.",
          });
        } finally {
          setLoading(false);
        }
      ...
      error: () => {
        ...
        setResult({
          error: true,
          message: "Invalid CSV format. Please upload a valid file.",
        });
      },
    });
  };
  ...
  const highestDisparity = result?.flagged_slices?.length
    ? Math.max(
        ...result.flagged_slices.map((slice) => slice.disparity_ratio || 0),
      ).toFixed(1)
    : "0.0";
  const riskLevel =
    flaggedCount === 0 ? "Low" : flaggedCount <= 2 ? "Moderate" : "High";
  const riskColor =
    flaggedCount === 0
      ? "var(--success)"
      : flaggedCount <= 2
    ...
    <section className="page-wrap">
      <header className="page-hero">
        ...
        <p className="hero-subtitle">
          Run batch-level statistical diagnostics on decision data, prioritize
          high-risk segments, and track compliance-ready evidence.
        </p>
      </header>
        ...
        <article className="panel">
          ...
          {!file ? (
            ...
            >
              ...
              <Upload
                size={28}
                style={{ marginBottom: "0.55rem", color: "var(--primary)" }}
              />
        ...
        <article className="panel">
          ...
          <div className="meta-line">
            <span className="muted-text">
              {domain === "lending" ? "gender" : "sex"}
            </span>
            <strong>
              {domain === "lending" ? "Male / Female" : "Male / Female / Other"}
            </strong>
          </div>
          <div className="meta-line">
            <span className="muted-text">
              {domain === "lending" ? "education" : "race"}
            </span>
            <strong>
              {domain === "lending"
          ...
          <div className="meta-line">
            <span className="muted-text">
              {domain === "lending" ? "income_group" : "age_group"}
            </span>
            <strong>
              {domain === "lending"
          ...
          <section className="info-box" style={{ marginTop: "0.9rem" }}>
            ...
            <p className="muted-text">
              Slices with small samples are flagged as low-confidence. Findings
              are statistical indicators and not legal determinations.
            </p>
          </section>
        </article>
      </div>
      ...
      {result && !result.error && (
        <>
          <section className="metric-grid">
            ...
            />
            <Metric
              label="Flagged Slices"
              value={flaggedCount}
              sub="demographic groups"
              color={flaggedCount > 0 ? "var(--danger)" : "var(--success)"}
            />
            <Metric
              label="Highest Disparity"
              value={`${highestDisparity}x`}
              sub="vs reference"
              color={
                parseFloat(highestDisparity) >= 3
                  ? "var(--danger)"
                  : "var(--warning)"
              }
            />
            ...
            {result.flagged_slices?.length > 0 ? (
              <div className="slice-list">
                {[...result.flagged_slices]
                  .sort(
                    (a, b) =>
                      (b.disparity_ratio || 0) - (a.disparity_ratio || 0),
                  )
                  .map((slice, index) => {
                    const priority = getPriorityClass(
                      slice.priority || slice.remediation_priority,
                    );
                    const approvalPct = Math.round(
                      (slice.approval_rate || 0) * 100,
                    );
                    const refPct = Math.round(
                      (slice.reference_approval_rate || 0) * 100,
                    );
                    const barColor =
                      priority === "high"
                        ? "var(--danger)"
                    ...
                    const sliceName =
                      domain === "lending"
                        ? `${slice.gender} ${slice.education}`
                        : `${slice.race} ${slice.sex}`;
                    const sliceDetail =
                      domain === "lending"
                      ...
                      <article key={keyId} className={`slice-card ${priority}`}>
                        <div className="slice-head">
                          ...
                          <div style={{ textAlign: "right" }}>
                            <p
                              className="slice-ratio"
                              style={{ color: barColor }}
                            >
                              {slice.disparity_ratio?.toFixed(1)}x
                            </p>
                            <span
                              className={`badge ${priority === "high" ? "error" : "success"}`}
                            >
                        ...
                        {slice.disparity_ratio < 0.8 && (
                          <section
                            className="info-box"
                            style={{ marginTop: "0.6rem" }}
                          >
                            <p className="info-label">Remediation Simulator</p>
                            <div className="meta-line">
                              <span className="muted-text">
                            ...
                            <div className="meta-line">
                              <span className="muted-text">
                                Additional approvals needed per 100
                              </span>
                              <strong>
                                {Math.round(
                                  (0.8 - (slice.approval_rate || 0)) * 100,
                                )}
                              </strong>
                            </div>
                          </section>
              ...
              <div className="empty-state">
                <CheckCircle
                  size={36}
                  style={{ marginBottom: "0.5rem", color: "var(--success)" }}
                />
                <p>
                  No statistically significant disparities detected in the
                  uploaded dataset.
                </p>
              </div>
            )}
      ...
      <p className="footer-note">
        Results are generated for operational risk review. Validate findings
        with governance, legal, and model risk stakeholders before remediation
        decisions.
      </p>
    </section>
  );
